				...
				u32 *ret_handle)
{
	...
	phys_pg_pack->arr = kcalloc(num_pgs, sizeof(*phys_pg), GFP_KERNEL);
	if (!phys_pg_pack->arr) {
		rc = -ENOMEM;
		goto pages_arr_err;
	}
	...
	if (phys_pg_pack->contiguous) {
		...
		for (i = 0 ; i < num_pgs ; i++) {
			phys_pg = &phys_pg_pack->arr[i];
			phys_pg->page_size = page_size;
			phys_pg->paddr = (u64) gen_pool_alloc(vm->dram_pg_pool,
							phys_pg->page_size);
			if (!phys_pg->paddr) {
				dev_err(hdev->dev,
 ...
 *
 * This function does the following:
 * - Iterate over the pack and free each physical block structure
		...
		struct hl_vm_phys_pg_pack **pphys_pg_pack)
{
	...
	phys_pg_pack->arr = kcalloc(total_npages, sizeof(*phys_pg),
					GFP_KERNEL);
	if (!phys_pg_pack->arr) {
		rc = -ENOMEM;
		...
		u64 *device_addr)
{
	...
	*device_addr = 0;
	...
	mapped_pg_cnt = map_phys_page_pack(ctx, ret_vaddr, phys_pg_pack);
	if (mapped_pg_cnt < phys_pg_pack->npages) {
	...
	*device_addr = ret_vaddr + phys_pg_pack->offset;
